...
> 이 글은 파이콘 한국 2019에서 파이썬 메타클래스에 대해 발표한 [리얼월드 메타클래스](https://www.pycon.kr/program/talk-detail?id=6)라는 발표의 텍스트 버전입니다. 슬라이드는 맨 아래에서 보실 수 있으며 영상 또한 파이콘 유튜브에 [공개되어 있습니다](https://www.youtube.com/watch?v=NEpVvBGPhLU). 
...
- 유명한 파이썬 라이브러리 Celery, Django에서 메타클래스가 어떻게 사용되는지(되었는지) 알아봅니다. 
...
지금까지 알아본 메타클래스는 과연 어디서 쓰이고 있을까요? 친숙한 오픈소스 라이브러리, 프레임워크들은 사실 오래 전부터 메타클래스를 활용하고 있습니다.

메타클래스를 사용하는 수많은 라이브러리와 프레임워크 중 여기서 살펴볼 것들은 `Celery` 와 `Django` 입니다.
