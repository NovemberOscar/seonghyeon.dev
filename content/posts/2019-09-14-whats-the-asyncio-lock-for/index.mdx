...
slug: whats-the-asyncio-lock-for
author: Seonghyeon Kim
date: 2019-09-14
hero: ./images/mutex-mutex-everywhere.jpg
excerpt: 파이썬 asyncio의 싱글스레드 환경에서의 뮤텍스 락에 대해서 예제 코드와 함께 살펴봅시다.
---
...
![request without lock](images/request-without-lock.png)
...
![request with lock](images/request-with-lock.png)

아까 전과는 다르게 첫번째 코루틴이 HTTP 요청을 하는 동안 나머지 코루틴은 락을 대기하면서 멈춰 있다.

![lock close up](images/lock-close-up.png)
...
![](images/more-request-without-lock-thread.png)

![](images/more-request-without-lock.png)
![](images/more-request-without-lock-2.png)

실행되는 스레드는 **30개**를 넘고 aiohttp가 만든 태스크로 인해 스크롤을 내려야만 다 확인할 수 있으며 실행시간은 약 두배인 **13.4초**정도가 걸렸다. (여러번 실행해 봤는데 가끔은 15초를 넘어갈 때도 있었다.)

락을 건 코드는 어떨까?

![](images/more-request-with-lock-thread.png)
![](images/more-request-with-lock.png)
